...
class _AuthenticateState extends State<Authenticate> {
  ...
  Widget build(BuildContext context) {
    ...
    return Chat.getNTPWrappedWidget(child);
  }
  ...
  _onPasscodeEntered(String enteredPasscode) {
    if (enteredPasscode.length == 4) {
      bool isValid = Chat.getHashedAnswer(enteredPasscode) == widget.passcode;
      ...
      if (isValid) {
        ...
        if (passcodeTries > 3) {
          Chat.toast('Try after ${math.pow(2, passcodeTries - 3)} minutes');
          Chat.toast('Authentication failed.');
          widget.state.pop();
        }
      }
    }
