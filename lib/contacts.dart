...
import 'package:chatapp/pre_chat.dart';
...
class _ContactsState extends State<Contacts>
  ...
  Future<Map<String, String>> getContacts({bool refresh = false}) {
    ...
    Chat.checkAndRequestPermission(PermissionGroup.contacts).then((res) {
      if (res) {
      ...
      if (this._searchIcon.icon == Icons.search) {
        ...
        this._appBarTitle = new TextField(
          ...
          decoration: new InputDecoration(
              hintText: 'Search ', hintStyle: TextStyle(color: White)),
        );
      } else {
  ...
  Widget build(BuildContext context) {
    super.build(context);
    return Chat.getNTPWrappedWidget(ScopedModel<DataModel>(
        model: widget.model,
                              ...
                              itemBuilder: (context, idx) {
                                ...
                                return ListTile(
                                  leading: CircleAvatar(
                                      radius: 22.5,
                                      child:
                                          Text(Chat.getInitials(user.value))),
