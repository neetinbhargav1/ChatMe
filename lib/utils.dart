...
class Chat{
  ...
  static void internetLookUp() {
    try {
      InternetAddress.lookup('example.com').catchError((e) {
        Chat.toast('No internet connection.');
      });
    } catch (_) {
      Chat.toast('No internet connection.');
    }
  ...
  static void invite() {
    Share.share(
        'Let\'s chat with me on encrypted ChatMe App, your all conversations are totally secure here, join me at - https://play.google.com/store/apps/details?id=neetin.aec.Votar&hl=en');
  }
      ...
      if (user[ALIAS_AVATAR] == null)
        return (user[PHOTO_URL] ?? '').isNotEmpty
            ...
            : CircleAvatar(
                ...
                child: Text(getInitials(Chat.getNickname(user))),
                radius: radius,
              );
      return CircleAvatar(
