import 'dart:async';
import 'dart:io';
import 'package:chatapp/EmojiPicker/emoji_picker.dart';
import 'package:chatapp/GiphyPicker/giphy_picker.dart';
import 'package:chatapp/photo_view.dart';
...
import 'package:chatapp/ImagePicker/image_picker.dart';
import 'package:chatapp/bubble.dart';
import 'package:chatapp/E2EE/e2ee.dart' as e2ee;
import 'package:chatapp/seen_provider.dart';
import 'package:chatapp/seen_state.dart';
...
import 'package:chatapp/crc.dart';
...
class _ChatScreenState extends State<ChatScreen> with WidgetsBindingObserver {
  ...
  dynamic encryptWithCRC(String input) {
    try {
      ...
      Chat.toast('Waiting for your peer to join the chat.');
      return false;
    }
  }
  ...
  readLocal() async {
    ...
    chatId = Chat.getChatId(currentUserNo, peerNo);
  ...
  contextMenu(Map<String, dynamic> doc, {bool saved = false}) {
    ...
    showDialog(
        context: context,
        builder: (context) {
          return Theme(data: ChatTheme, child: SimpleDialog(children: tiles));
        });
  }
  ...
  deleteMessagesUpto(int upto) {
    if (upto != null) {
      ...
      setState(() {
        ...
        if (messages.length < before) Chat.toast('Conversation Ended!');
      });
    }
  }
  ...
  Widget getTextMessage(bool isMe, Map<String, dynamic> doc, bool saved) {
    return Text(
      doc[CONTENT],
      style:
          TextStyle(color: isMe ? White : Colors.black, fontSize: 16.0),
    );
  }
      ...
      child: Row(
        children: <Widget>[
          IconButton(
              ...
              onPressed: () async {
                final emoji = await GiphyPicker.pickGif(
                    context: context,
                    apiKey: 'PkjPKUvd84HUEd2GGStxDxW8za02HBti');
                onSendMessage(emoji.images.original.url, MessageType.image,
                          ...
                          appBar: PreferredSize(
                            ...
                            child: Material(
                              child: ListTile(
                                  ...
                                  dense: true,
                                  leading: Chat.avatar(peer),
                                  title: Text(
                                    Chat.getNickname(peer),
                                    style: TextStyle(
                                        color: White,
                                        fontSize: 16.0,
                                ...
                                decoration: new BoxDecoration(
                                  image: new DecorationImage(
                                      image: peer[WALLPAPER] == null
                                          ? AssetImage("assets/background1.jpeg")
