...
import 'package:chatapp/security.dart';
...
class LoginScreenState extends State<LoginScreen>
  ...
  Future<void> verifyPhoneNumber() async {
    ...
    await firebaseAuth.verifyPhoneNumber(
        phoneNumber: (phoneCode + _phoneNo.text).
        trim(),
  ...
  Future<Null> handleSignIn({AuthCredential authCredential}) async {
    ...
    try {
      firebaseUser = (await firebaseAuth
        ...
        await Chat.reportError(err, 'signInWithCredential');
        Chat.toast(
      ...
      await Chat.reportError(e, 'signInWithCredential catch block');
      Chat.toast(
          'Make sure your Phone Number/OTP Code is correct and try again later.');
      if (mounted)
        setState(() {
    ...
    if (firebaseUser != null) {
      ...
      if (documents.isEmpty) {
        ...
        unawaited(Navigator.pushReplacement(context,
            new MaterialPageRoute(builder: (context) => ChatWrapper())));
        Chat.toast('Welcome back!');
      }
    } else {
      Chat.toast("Failed to log in.");
    }
  }
          ...
          children: <Widget>[
            Center(
              child: new Stepper(
                ...
                steps: <Step>[
                  new Step(
                    title: Text('Phone Code',  style: TextStyle( fontSize:16 ),),
                            ...
                            RaisedButton(
                                ...
                                onPressed: () {
                                  ...
                                  if (_name.text.trim().isNotEmpty) {
                                    ...
                                    Chat.toast('Name cannot be empty!');
                                  }
                                },
                                child: Text('Next')),
                            SizedBox(width: 8.0),
                  ...
                  new Step(
                    title: Text('Verify OTP', style: TextStyle( fontSize:16 ),),
